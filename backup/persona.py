...
def create_personas_tab() -> None:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            with gr.Row():
                save_btn = gr.Button("Save Persona", variant="primary", size="lg")
                delete_btn = gr.Button("Delete Persona", variant="stop", size="lg")
                preview_btn = gr.Button("Preview Voice", size="lg")
    ...
    personas_gallery = gr.HTML(value=_render_persona_cards(list_personas()))
    ...
    def on_voice_select(
        voice_value: str,
    ) -> tuple[str, str, str | None, str | None, str, str, str, str, bool]:
    ...
    def on_save_persona(
        ...
        if not char_name or not char_name.strip():
            ...
            return "Error: Character name required", _render_persona_cards(
                list_personas()
            )
        ...
        if not style:
            ...
            return "Error: Speaking style required", _render_persona_cards(
                list_personas()
            )
    ...
    def on_delete_persona(
        voice_value: str, confirm_state: bool
        ...
        if not confirm_state:
            gr.Warning(
                f"⚠️ Click Delete again to confirm deletion of persona for '{voice_id}'"
            )
            return (
    ...
    refresh_voices_btn.click(fn=on_refresh_voices, outputs=[voice_dropdown])

    refresh_gallery_btn.click(fn=on_refresh_gallery, outputs=[personas_gallery])
