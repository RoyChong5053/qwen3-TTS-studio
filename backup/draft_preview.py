...
:root {
    ...
    --accent-blue: #8080ff;
    --accent-green: #50c878;
    --accent-orange: #ffb432;
    --radius: 6px;
    
    /* Slate aliases for compatibility */
    --slate-50: var(--bg-dark);
    --slate-100: var(--bg-medium);
    --slate-200: var(--bg-light);
    --slate-300: var(--border-light);
    --slate-400: var(--border);
    --slate-500: var(--text-dim);
    --slate-600: var(--text-muted);
    --slate-700: var(--text-muted);
    --slate-800: var(--text-bright);
    --slate-900: var(--text-bright);
    
    /* Accent aliases */
    --amber-100: rgba(255, 180, 50, 0.15);
    --amber-200: rgba(255, 180, 50, 0.25);
    --amber-500: var(--accent-orange);
    --emerald-100: rgba(80, 200, 120, 0.15);
...
def format_segment_header(segment: Segment, idx: int, is_edited: bool) -> str:
    ...
    return (
        f"{idx + 1}. {segment.title} {get_size_badge_html(segment.size)}{edited_badge}"
    )


def render_dialogues_html(dialogues: list[Dialogue], edited_indices: set[int]) -> str:
...
def get_segment_dialogues(state: dict[str, Any], segment_idx: int) -> list[Dialogue]:
...
def on_save_segment_edit(
    ...
    if outline and segment_idx < len(outline.segments):
        ...
        if (
            new_title.strip() != segment.title
            or new_desc.strip() != segment.description
        ):
            # Update segment
...
def on_save_dialogue_edit(
    ...
    if segment_idx in transcripts:
        ...
        if 0 <= editing_idx < len(dialogues):
            ...
            if (
                new_speaker.strip() != old_dialogue.speaker
                or new_text.strip() != old_dialogue.text
            ):
                # Update dialogue
    ...
    with gr.Blocks(css=DRAFT_PREVIEW_CSS, title="Draft Preview") as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                gr.HTML(
                    '<div style="margin-top: 1rem; font-size: 0.85rem; color: var(--slate-500);">Click a segment to edit:</div>'
                )
                with gr.Row():
                    seg_btns = []
                    for i in range(5):
                ...
                gr.HTML(
                    '<div class="panel-header" style="margin-top: 1.5rem;">Edit Segment</div>'
                )
            ...
            with gr.Column(scale=3):
                ...
                with gr.Column(elem_classes=["dialogue-panel"]):
                    ...
                    # Dialogue selection
                    gr.HTML(
                        '<div style="margin-top: 1rem; font-size: 0.85rem; color: var(--slate-500);">Select dialogue to edit:</div>'
                    )
