...
def calculate_overall_progress(step: GenerationStep, step_progress: float) -> float:
    ...
    completed_weight = sum(STEP_WEIGHTS[s] for s in steps[: steps.index(step)])
...
def create_step_indicator_html(
    current_step: GenerationStep, step_progress: float = 0.0
) -> str:
    """
...
@keyframes pulse {
    ...
    50% {
        box-shadow: 0 0 0 8px rgba(128, 128, 255, 0);
    }
}
...
def create_demo_app():
    ...
    with gr.Blocks(title="Progress UI Demo") as demo:
        ...
        with gr.Column():
            ...
            step_indicator, overall_progress, status_text, time_remaining = (
            ...
            with gr.Row():
                start_demo_btn = gr.Button(
                    "Start Mock Generation", variant="primary", size="lg"
                )
            ...
            for i in range(4):
                ...
                yield (
                    create_step_indicator_html(
                        GenerationStep.TRANSCRIPT, segment_progress
                    ),
                ...
                for clip_num in range(num_clips):
                    ...
                    state = ProgressState(
                        ...
                        overall_progress=calculate_overall_progress(
                            GenerationStep.AUDIO, step_progress
                        ),
                        status_text="",
                    ...
                    yield (
                        ...
                        "\n".join(
                            log_lines + [f"  - Clip {clip_idx}/{total_clips}: {title}"]
                        ),
                    )
