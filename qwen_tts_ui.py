...
def history_tab_delete(choice, confirm_state, query, favorites_only, tab_filter):
    ...
    if confirm_state and result[0] == "Deleted":
        filter_val = (
            tab_filter.lower() if tab_filter and tab_filter != "All" else "voice"
        )
        display = format_history_for_display(query, favorites_only, filter_val)
        choices = get_history_choices(filter_val, query, favorites_only)
        return (
...
custom_css = (Path(__file__).parent / "static" / "dark-theme.css").read_text()
...
PRESET_VOICES = [
    {
        "voice_id": "aiden",
        "name": "Aiden",
        "desc": "Bright American male, clear midrange",
    },
    ...
    {"voice_id": "uncle_fu", "name": "Uncle Fu", "desc": ""},
    {
        "voice_id": "vivian",
        "name": "Vivian",
        "desc": "Bright, slightly sharp young female",
    },
]
